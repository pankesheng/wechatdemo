...
public class RedisUtils2 {
	...
	/**
	 * 字符串类型: 返回key对应的字符串value的子串，自个子串是由start和end位移决定的（两者都在string内）。
	...
	/**
	 * 字符串类型: 对存储在指定key的数值执行原子的加1操作。 如果指定的key不存在，那么在执行incr操作之前，会先把它的值设定为0.
	 * 如果指定的key中存储的值不是字符串类型或者存储的字符串类型不能表示为一个整数，那么执行这个命令时服务器会返回一个错误。
	 * 注意：由于redis并没有一个明确的类型来表示整型数据，所以这个操作是一个字符串操作。
	 * 执行这个操作的时候，key对应存储的字符串被解析为10进制的64位有符号整型数据。 这个操作仅限于64位的有符号整型数据。
	...
	/**
	 * 字符串类型: 对key对应的数字做减一操作。如果key不存在，那么在操作之前，这个key对应的值会被设定为0。
	 ...
	 * 如果你不想覆盖已经存在的values，那么需要参考msetnx. mset是原子性的，所以所有给定的keys是一次性set的。
	...
	/**
	 * 字符串类型: 对应给定的keys到他们相应的values上。只要有一个values已经存在，MSETNX一个操作都不会执行。
	 * 由于这种特性，MSETNX可以实现要么所有的操作都成功， 要么一个都不执行，这个可以用来设置不同的key，来表示一个唯一的对象的不同字段。
	...
	/**
	 * list列表类型: 返回存储在key的列表里指定范围内的元素。Start和end偏移量都是基于0的下标，
	...
	/**
	 * set无序集合类型: 把两个集合的交集结果集结果保存到一个新的集合中。如果这个新的集合是已经存在的，那么这个新的集合则会被重写。
	 * 返回值为结果集中成员的个数。
	 * 
	 * @param key
	 * @param bytes
	...
	public Boolean smove(final String key1, final String key2,
			final String field) {
		return redisMasterTemplate.execute(new RedisCallback<Boolean>() {
			
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.sMove(getRedisMasterSerializer()
						.serialize(key1),
	...
	public Boolean sismember(final String key, final String value) {
		return redisSlaveTemplate.execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.sIsMember(getRedisSlaveSerializer()
						.serialize(key),
	...
	public Boolean zadd(final String key, final Double offset,
			final byte[] bytes2) {
		return redisMasterTemplate.execute(new RedisCallback<Boolean>() {
			
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.zAdd(getRedisMasterSerializer()
						.serialize(key), offset, bytes2);
			}
		});
	}
	...
	public Set<byte[]> zrange(final String key, final Long start, final Long end) {
		return redisSlaveTemplate.execute(new RedisCallback<Set<byte[]>>() {
			...
			public Set<byte[]> doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.zRange(getRedisSlaveSerializer()
						.serialize(key), start, end);
			}
		});
	}
	...
	public Double zincrby(final String key, final Double offset,
			final String field) {
		return redisMasterTemplate.execute(new RedisCallback<Double>() {
			
			public Double doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.zIncrBy(getRedisMasterSerializer()
						.serialize(key), offset, getRedisMasterSerializer()
						.serialize(field));
			}
		});
	}
	...
	public Set<byte[]> zrangebyscore(final String key, final Double start,
			final Double end) {
		return redisSlaveTemplate.execute(new RedisCallback<Set<byte[]>>() {
			
			public Set<byte[]> doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.zRangeByScore(getRedisSlaveSerializer()
						.serialize(key), start, end);
			}
		});
	}
	...
	public Long zremrangebyrank(final String key, final Double start,
			final Double end) {
		return redisMasterTemplate.execute(new RedisCallback<Long>() {
	...
	/**
	 * hash类型: 设置 key 指定的哈希集中指定字段的值 如果 key 指定的哈希集不存在，会创建一个新的哈希集并与 key 关联
	 * 如果字段在哈希集中存在，它将被重写
	 * 
	 * @param key
	 * @param field
	...
	public Boolean hset(final String key, final String field, final String value) {
		return redisMasterTemplate.execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hSet(getRedisMasterSerializer()
						.serialize(key),
						getRedisMasterSerializer().serialize(field),
	...
	public byte[] hget(final String key, final String field) {
		return redisSlaveTemplate.execute(new RedisCallback<byte[]>() {
			...
			public byte[] doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hGet(getRedisSlaveSerializer()
						.serialize(key),
	...
	public Set<byte[]> hkeys(final String key) {
		return redisSlaveTemplate.execute(new RedisCallback<Set<byte[]>>() {
			...
			public Set<byte[]> doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hKeys(getRedisSlaveSerializer()
						.serialize(key));
			}
		});
	}
	...
	public Long hdel(final String key, final byte[]... fields) {
		return redisMasterTemplate.execute(new RedisCallback<Long>() {
			...
			public Long doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hDel(getRedisMasterSerializer()
	...
	public Long hlen(final String key) {
		return redisSlaveTemplate.execute(new RedisCallback<Long>() {
			...
			public Long doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hLen(getRedisSlaveSerializer()
						.serialize(key));
			}
		});
	}
		...
		return redisMasterTemplate.execute(new RedisCallback<Void>() {
			...
			public Void doInRedis(RedisConnection redisConnection)
				...
				redisConnection.hMSet(
						getRedisMasterSerializer().serialize(key), map);
				return null;
			}
		});
	}
	...
	public List<byte[]> hmget(final String key, final byte[]... fields) {
		return redisSlaveTemplate.execute(new RedisCallback<List<byte[]>>() {
			...
			public List<byte[]> doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hMGet(getRedisSlaveSerializer()
						.serialize(key), fields);
			}
		});
	}
	...
	/**
	 * hash类型: 增加 key 指定的哈希集中指定字段的数值。 如果 key 不存在，会创建一个新的哈希集并与 key
	 * 关联。如果字段不存在，则字段的值在该操作执行前被设置为 0 HINCRBY 支持的值的范围限定在 64位 有符号整数
	 * 
	 * @param key
	 * @param field
	 * @param val
	...
	public Double hincrby(final String key, final String field, final Double val) {
		return redisMasterTemplate.execute(new RedisCallback<Double>() {
			...
			public Double doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hIncrBy(getRedisMasterSerializer()
						.serialize(key),
						getRedisMasterSerializer().serialize(field), val);
			}
		});
	}
	...
	public Boolean hexists(final String key, final String field) {
		return redisSlaveTemplate.execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hExists(getRedisSlaveSerializer()
						.serialize(key),
						getRedisSlaveSerializer().serialize(field));
			}
		});
	...
	public List<byte[]> hvals(final String key) {
		return redisSlaveTemplate.execute(new RedisCallback<List<byte[]>>() {
			...
			public List<byte[]> doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hVals(getRedisSlaveSerializer()
						.serialize(key));
			}
		});
	}
	...
	/**
	 * hash类型: 只在 key 指定的哈希集中不存在指定的字段时，设置字段的值。 如果 key 指定的哈希集不存在，会创建一个新的哈希集并与 key
	 * 关联。 如果字段已存在，该操作无效果。
	 * 
	 * @param key
	 * @param field
	...
	public Boolean hsetnx(final String key, final String field,
		...
		return redisMasterTemplate.execute(new RedisCallback<Boolean>() {
			...
			public Boolean doInRedis(RedisConnection redisConnection)
					throws DataAccessException {
				return redisConnection.hSetNX(getRedisMasterSerializer()
						.serialize(key),
						getRedisMasterSerializer().serialize(field),
						getRedisMasterSerializer().serialize(value));
			}
				...
				.execute(new RedisCallback<Map<byte[], byte[]>>() {
					...
					public Map<byte[], byte[]> doInRedis(
						...
						return redisConnection
								.hGetAll(getRedisSlaveSerializer().serialize(
										key));
					}
				});
	}
	...
	protected RedisSerializer<String> getRedisMasterSerializer() {
		RedisSerializer<String> redisSerializer = getRedisMasterTemplate()
				.getStringSerializer();
	...
	protected RedisSerializer<String> getRedisSlaveSerializer() {
		RedisSerializer<String> redisSerializer = getRedisSlaveTemplate()
				.getStringSerializer();
		return redisSerializer;
	}
